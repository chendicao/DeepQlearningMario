...
class Level1(tools._State):
    ...
    def update(self, surface, keys, current_time):
        ...
        image, reward = self.blit_everything(surface)
        self.sound_manager.update(self.game_info, self.mario)
        return image, reward, self.game_info[c.MARIO_DEAD]
    ...
    def blit_everything(self, surface):
        ...
        if self.last_x_position <= self.save_x_position:
          reward = 0
        else:
          reward = 1
        self.save_x_position = self.last_x_position
        scores = self.game_info[c.SCORE]
        image = cv2.cvtColor(pg.surfarray.array3d(surface), cv2.COLOR_RGB2GRAY)
        resized_image = cv2.resize(image, (80, 80)) 
        # gray scale
        return resized_image, reward
        # to save
        # cv2.imwrite('fuckingmario.png', imgdata)
